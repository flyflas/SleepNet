...
class Transformer(nn.Module):
    def __init__(self, config):
        ...
        self.position_single = PositionalEncoding(
            d_model=config.dim_model, dropout=0.1, max_len=config.pad_size + 1
        )
        self.position_multi = PositionalEncoding(
            d_model=config.dim_model * 3, dropout=0.1, max_len=config.pad_size + 1
        )
    ...
    def forward(self, x):
        ...
        x1_eog = self.cross_eeg1_eog(query=x1, key=x3, value=x3)
        x2_eog = self.cross_eeg2_eog(query=x2, key=x3, value=x3)

        x = torch.cat([x1_eog, x2_eog, x3], dim=2)
